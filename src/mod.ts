...
class AutoCompatFramework implements IPostDBLoadMod
{
    public postDBLoad(container: DependencyContainer): void
    {
        ...
        const processCompatibility = (passNumber: number, modifiedItems?: Set<string>) => 
        {
            ...
            // Caliber to ammo IDs: caliber => [ammoIds] (both base and modded)
            ...
            // Function to check if a slot is proprietary (only accepts modded IDs or empty)
            ...
            // Apply compatibility
            // 1. Add modded ammo to chambers/cartridges of weapons/mags if caliber matches and not proprietary
            for (const weapon of weaponsToProcess) 
            {
                ...
                if (caliber && caliberToAmmo.has(caliber)) 
                {
                    const moddedAmmoForCaliber = caliberToAmmo.get(caliber)!.filter(ammoId => 
                    {
                        ...
                        return !prefabPath.startsWith("assets/content/") && !modConfig.blacklist.includes(ammoId);
                    });
                    const chambers = weapon._props.Chambers || [];
                    for (const chamber of chambers) 
                    {
                        const filter = chamber._props.filters?.[0]?.Filter || [];
                        const isProprietary = isProprietarySlot(filter) && !modConfig.excludeFromProprietary.includes(weapon._id);
                        if (!isProprietary) 
                        {
                    ...
                    // Similar for cartridges in magazines
            ...
            // 2. Add modded attachments to slots if the slot accepts the base attachment and not proprietary
            for (const slottedItem of slottedItemsToProcess) 
            {
                ...
                if (slotsMap) 
                {
                    ...
                    {
                        const isProprietary = isProprietarySlot(filter) && !modConfig.excludeFromProprietary.includes(slottedItem._id);
                        ...
                        for (const acceptedId of [...filter]) 
                        {
                            ...
                            {
                                const clones = baseToClones.get(acceptedId)!.filter(cloneId => !modConfig.blacklist.includes(cloneId));
            ...
            // 3. Push conflicting items from base to clones
            ...
            // 4. Process ManualAdd entries (if any)(I kinda don't know if this is gonna work, but, yolo)
            for (const manual of modConfig.ManualAdd || []) 
            {
                ...
                // Find matching sluts on target item to take it
                ...
                const isProprietary = isProprietarySlot(targetFilter) && !modConfig.excludeFromProprietary.includes(targetItemId);
                if (isProprietary) 
                {
                    if (modConfig.verboseLogging) 
                    {
            ...
            if (!modConfig.verboseLogging) 
            {
                logger.info(`AutoCompatFramework Pass ${passNumber} Summary: Added ${numAmmoToChambers} ammo to chambers, ${numAmmoToCartridges} ammo to cartridges, ${numAttachmentsToSlots} attachments to slots, ${numBaseConflictsAdded} base conflicts, ${numClonedConflictsAdded} cloned conflicts, ${numConflictsVoided} conflicts voided, ${numManualAdditions} manual additions.`);
            }
        ...
        {
            logger.info(`AutoCompatFramework Total Summary: Added ${totalAmmoToChambers} ammo to chambers, ${totalAmmoToCartridges} ammo to cartridges, ${totalAttachmentsToSlots} attachments to slots, ${totalBaseConflictsAdded} base conflicts, ${totalClonedConflictsAdded} cloned conflicts, ${totalConflictsVoided} conflicts voided, ${totalManualAdditions} manual additions.`);
        }
